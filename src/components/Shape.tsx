...
const Shape: React.FC<ShapeProps> = ({
  ...
  const getShapeClass = () => {
    ...
    switch (shape.type) {
      ...
      case 'diamond':
        return baseClass + "bg-white" + (shape.type === 'diamond' ? ' rotate-45' : '');
      case 'text':
  ...
  const handleControlPointDrag = (pointType: 'cp1' | 'cp2', position: Point) => {
    if (shape.type === 'line') {
      ...
      if (!updatedShape.controlPoints) {
        const midX = (shape.position.x + (shape.endPoint?.x || 0)) / 2;
        const midY = (shape.position.y + (shape.endPoint?.y || 0)) / 2;
        updatedShape.controlPoints = {
          cp1: { x: midX, y: midY - 50 },
          cp2: { x: midX, y: midY - 50 }
        };
      }
      updatedShape.controlPoints[pointType] = position;
      onDrag(shape.id, position, false, updatedShape);
    }
  };
        ...
        {isSelected && (
          <>
            ...
            >
              <div
                ref={nodeRef}
                className="absolute w-3 h-3 bg-white border-2 border-gray-400 rounded-full cursor-move"
